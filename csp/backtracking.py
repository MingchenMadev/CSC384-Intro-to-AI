        ...
        for constraint in csp.constraintsOf(var):
            if constraint.numUnassigned() == 1:
                if FCCheck(constraint, var, val) == "DWO":
                    ...
                    if trace: print "<==DWO\n"
                    break
        # just modify this question as a modification of the BT
        #  change it by the sudo code
        if noDWO:
            ...
            if len(solns) > 0 and not allSolutions:
                ...
                break #don't bother with other values of nxtvar
                      #as we found a soln.
        var.restoreValues(var, val)#as of the sudo code by the slides we need to restore this variable
    var.setValue(None)
...
def GacEnforce(constraints, csp, reasonVar, reasonVal):
    ...
    while not constraints == []:
      cnstr = constraints.pop() #make cnstr GAC for var in cnstr.scope():
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in var.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(var),csp, var, val) == "DWO":
          noDWO = False
        # using BT code and sudo code and remodify them
        if noDWO:
