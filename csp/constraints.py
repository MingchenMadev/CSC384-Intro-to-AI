        ...
        for x in qi._dom:
          for y in qj._dom:
            if x != y and abs(i - j) != abs(x - y):
              ans.append([x,y])
        # append the contrainst list to match the need for my tableConstarint
        TableConstraint.__init__(self, name, [qi,qj], ans)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def valsBetweenLbUb(l):
            return self._lb <= len([val for var,val in l if val == self._required])<= self._ub

        def valsLowerUb(l):
            return len([val for var,val in l if val == self._required])<= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsBetweenLbUb, valsLowerUb)
        return x
