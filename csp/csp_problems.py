...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            scope = row
            constraint_list.append(AllDiffConstraint("row", scope))
            # append the alldiffconstraint result to the list
            # for all the require to write
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("subs", scope))
